...
for repeat=1:max_num
    ...
    for i=1:K
        ...
        dt=t(2)-t(1);
        dx=x(2)-x(1);
        u00=[sech(x);3*sech(x);exp(-(x+2).^2);0.2*exp(-(x+2).^2);sin(pi*x);2*sech(x);...
            0.3*exp(-(x+2).^2); 2*sin(pi*x); 0.5*sin(pi*x);cos(x);sech(x);sech(x);sech(x)];
        for r=1:n
            u(1,r)=u00(i,r)+0.02*(2*rand(1,1)-1);%u00(i,r)+
        end
        %u=sech(x)+0.01*(2*rand(1,n)-1); % initial conditions
        ut=fft(u);
    ...
    figure(1)
    mesh(abs(usol));
    data{i} = usol([1,end],:); % record the initial time and final time
    end
...
%  c11=-0.1014+10.3631*b+9.4487*b-89.249*b*b=1.9664e-04;
%  c22=10.3631*a-89.249*a*b*2=0.9973;
%  c33=10.3631*a=1.0965;
%  c44=-89.249*a*a=-0.9991;